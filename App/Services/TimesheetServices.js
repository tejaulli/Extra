...
class TimeSheetServices {
    create_timesheet(user) {
        ...
        return axios.post(USER_API_BASE_URL + 'create-timesheet/', user, axiosConfig);
    ...
    viewEmployeeTimeSheet(orgId,userId) {
        ...
        return axios.get(USER_API_BASE_URL + 'view-emp-timesheet/?orgId=' + orgId + '&userId=' + userId);
    }
    get_timesheet(orgId,userId,inputdate) {
        return axios.get(USER_API_BASE_URL + 'get-timesheet-details/?orgId='+ orgId +'&userId=' + userId + '&date=' + inputdate);

    }
	
	//Login Api
	login_api(email,password){
		return axios.post(USER_API_BASE_URL + 'user/login/?email=' + email + '&password=' + password);
	}
	
	forget_password(email){
    return axios.get(USER_API_BASE_URL + 'user/forget-password/?email=' + email);
	}
	
	change_password(email,currentPassword,newPassword){
    return axios.get(USER_API_BASE_URL + 'user/change-password/?email=' + email + '&currentPassword=' + currentPassword  + '&newPassword=' + newPassword);
	}
